...
final class VuforiaCheckerThread extends Thread {
    ...
    VuforiaCheckerThread(long checkingInterval, VuforiaCheckerThreadOutputType outputType, CVCompatibleClass caller) {
        this.stopped = false;
        this.checkAtIntervalsOf = checkingInterval;
        if (outputType == VuforiaCheckerThreadOutputType.STREAM) {
